...
const GeneratePost: React.FC = () => {
  ...
  const [postBunch, setPostBunch] = useState<PostBunch | null>(null);
...
#DigitalTransformation #Leadership #Innovation #TechnologyStrategy`;
  ...
  const fetchPostBunch = async (clerkRef: string) => {
    try {
      const response = await fetch('http://localhost:3000/api/post/getpostbunch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clerkRef })
      });
      ...
      console.error('Error fetching post bunch:', error);
      return { success: false, error: (error as Error).message };
    }
  };

  const createNewPostBunch = async (clerkRef: string) => {
    try {
      const response = await fetch('http://localhost:3000/api/post/createpostbunch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clerkRef })
      });
      ...
      console.error('Error creating post bunch:', error);
      return { success: false, error: (error as Error).message };
    }
  };

  const updatePostBunchTitles = async (clerkRef: string, retries = 3) => {
    for (let i = 0; i < retries; i++) {
      try {
        const response = await fetch('http://localhost:3000/api/post/updatepostbunch', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ clerkRef })
        });
        ...
        if (i === retries - 1) {
          console.error('Error updating post bunch:', error);
          return { success: false, error: (error as Error).message };
        }
  ...
  useEffect(() => {
    const initializePosts = async () => {
      ...
      setIsLoading(true);
      try {
        // First, try to get existing post bunch
        const getResult = await fetchPostBunch(user.id);
        
        if (!getResult.success) {
          if (getResult.error) {
            console.error('Error:', getResult.error);
          }
          // If no post bunch exists, create new one
          const createResult = await createNewPostBunch(user.id);
          if (createResult.success) {
            setPostBunch(createResult.postBunch);
            setPosts(createResult.postBunch.posts);
          } else {
            console.error('Failed to create post bunch:', createResult.error);
          }
        } else {
          // If post bunch exists, update titles
          const updateResult = await updatePostBunchTitles(user.id);
          if (updateResult.success) {
            setPostBunch(updateResult.postBunch);
            setPosts(updateResult.postBunch.posts);
          } else {
            console.error('Failed to update post bunch:', updateResult.error);
          }
        }
      } catch (error) {
  ...
  // Update current post content when posts or currentPostIndex changes
  useEffect(() => {
    if (posts.length > 0 && currentPostIndex < posts.length) {
      setPostContent(posts[currentPostIndex].content || posts[currentPostIndex].title);
    }
  ...
  useEffect(() => {
    ...
    socket.on('postGenerated', (data: { title: string; content: string }) => {
  ...
  useEffect(() => {
    if (posts.length > 0) {
      ...
      setGeneratingPosts(titles);
    }
  }, [posts.length]);

  // Modify the AI-Generated Post Card section
  const renderPostContent = () => {
    const currentPost = posts[currentPostIndex];
    if (!currentPost) return null;

    if (generatingPosts.has(currentPost.title)) {
      return (
            ...
            <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center bg-gray-100 rounded-full px-2 py-1">
                  ...
                  <span className="mx-4 text-sm font-medium text-gray-700">Post {currentPostIndex + 1}/{posts.length}</span>
                  <button
